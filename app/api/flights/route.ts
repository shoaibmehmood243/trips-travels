...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!departureAirport) {
      return NextResponse.json({ success: false, message: "Departure airport required" }, { status: 400 });
    }
    if (!destination) {
      return NextResponse.json({ success: false, message: "Destination airport required" }, { status: 400 });
    }
    if (!departureDate) {
      return NextResponse.json({ success: false, message: "Departure date required" }, { status: 400 });
    }

    // Send email using Resend
    const resendApiKey = process.env.RESEND_API_KEY;
    const flightsEmail = process.env.FLIGHTS_EMAIL || "flights@tripstravels.co.uk";
    const fromEmail = process.env.FROM_EMAIL || "noreply@tripstravels.co.uk";

    const emailSubject = `New Flight Booking Enquiry from ${name}`;
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; background-color: #ffffff;">
        <div style="background-color: #0A1628; padding: 24px; text-align: center; border-bottom: 3px solid #C9932A;">
          ...
          <p style="color: #7A8BA6; margin: 4px 0 0 0; font-size: 14px;">Detailed Flight Booking Enquiry</p>
        </div>
        <div style="padding: 24px; color: #0A1628;">
          <h2 style="font-size: 18px; margin-top: 0; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px; color: #C9932A;">Customer & Flight Details</h2>
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            ...
            <tr>
              <td style="padding: 8px 0; font-weight: bold;">Departure Airport:</td>
              <td style="padding: 8px 0;">${departureAirport}</td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td style="padding: 8px 0; text-transform: capitalize;">${cabinClass}</td>
            </tr>
            <tr>
            ...
            <tr>
              <td style="padding: 8px 0; font-weight: bold;">Referral Source:</td>
              <td style="padding: 8px 0; text-transform: capitalize;">${source || "N/A"}</td>
            </tr>
          </table>
          ...
          ${
            message
              ? `
            <h2 style="font-size: 18px; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px; color: #C9932A; margin-top: 24px;">Additional Notes</h2>
    ...
    if (resendApiKey) {
      ...
      console.log("=== FLIGHTS EMAIL WOULD BE SENT ===");
      console.log(`To: ${flightsEmail}`);
      console.log(`From: ${fromEmail}`);
      console.log(`Subject: ${emailSubject}`);
      console.log("====================================");
    }

    return NextResponse.json({ success: true, message: "Flight enquiry received successfully." });
  } catch (error) {
